...
function preloadVideo(src) {
    ...
    video.src = src;
    video.preload = 'auto';
    video.muted = true; // Required for autoplay policies, good for preloading
    video.load(); // Start loading
    mediaCache.videos.push(video);
}

function preloadProjectMedia(project) {
    if (project.images && project.images.length > 0) {
        project.images.forEach(img => preloadImage(img));
    }
    ...
    if (project.videoMobile) {
        preloadVideo(project.videoMobile);
    }
}

// 1. Preload first project immediately (High Priority)
if (projects.length > 0) {
    preloadProjectMedia(projects[0]);
}

// 2. Preload remaining projects after window load (Background Priority)
window.addEventListener('load', () => {
    // Small delay to ensure main thread is free
    setTimeout(() => {
        for (let i = 1; i < projects.length; i++) {
            preloadProjectMedia(projects[i]);
        }
    }, 1000);
});
